...
for key, value in methods_jdata.items():
    ...
    extracted_method['title'] = value['title']
    extracted_method['id'] = value['id']
    extracted_method['type'] = value['type']
    extracted_method['ownerName'] = value['ownerName']
    extracted_method['ownerEmail'] = value['ownerEmail']

    # print('Key: ', value['url'], '\n\t Value: ', result_value)
    save_path = 'json_data/methods/'
    os.makedirs('json_data/methods/', exist_ok = True)
    json_data = json.dumps(extracted_method, indent=2)
    file_name = value['title'].replace(':', '')
