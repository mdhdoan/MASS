...
with open('json_data/protocols_jdata.json', 'r') as protocol_file:
...
for key, value in protocols_jdata.items():
    ...
    result_value = result_value.replace('\"', '')
    extracted_protocol['background'] = result_value
    
    extracted_protocol['url'] = value['url']
    ...
    extracted_protocol['metrics'] = value['metrics']
    ...
    extracted_protocol['programUrl'] = value['programUrl']

    # print('Key: ', value['url'], '\n\t Value: ', result_value)
    save_path = 'json_data/protocols/'
    os.makedirs('json_data/protocols/', exist_ok = True)
    json_data = json.dumps(extracted_protocol, indent=2)
    file_name = extracted_protocol['title'].replace(':', '')
